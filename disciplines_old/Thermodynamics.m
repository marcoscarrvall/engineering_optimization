function [A_inlet, A_core, TIT, thrust, TSFC, mdot_total, fuel_flow] = Thermodynamics(data, design_vec, thrust_N)
% THERMODYNAMICS  Two-spool two-stream turbofan cycle solver for MDA loops.
%
%   [A_inlet, A_core, TIT, thrust, TSFC, mdot_total, fuel_flow] = Thermodynamics(data, design_vec, thrust_N)
%
...
% -------------------------------------------------------------------------
    ...
    if F_sp <= 0
        error('Thermodynamics:noThrust', ...
              'Cycle produces non-positive specific thrust (%.3f N.s/kg).', F_sp)
    end
    ...
    % --- TSFC ------------------------------------------------------------
    fuel_flow = fuel_flow_sp * mdot_total;        % kg/s actual
    TSFC      = fuel_flow / thrust_N * 1e6;       % g/kNs
...
%CORE_CYCLE  Two-spool core thermodynamic cycle.
    ...
    W_fan_full = 1.0 * data.cp_air * (T21 - T2);  
